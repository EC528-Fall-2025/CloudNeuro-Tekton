...
def render(workflow: Workflow) -> str:
    """
    Render the workflow as a bash script that submits one SLURM job per Tekton task.

    - Each Tekton Task becomes a single `sbatch` job.
    - Task `run_after` dependencies become `--dependency=afterok:<job-ids>`.
    """
    lines: List[str] = [
        "#!/usr/bin/env bash",
        "set -euo pipefail",
        f'echo "Submitting SLURM workflow: {workflow.name}"',
        "",
    ]

    # Map Tekton task name -> bash variable holding its SLURM job ID
    ...
    for task in workflow.tasks:
        ...
        if deps:
            ...
            dep_opt = f"--dependency=afterok:{dep_expr} "

        lines.append(f'echo "Submitting task {task.name}..."')

        # Capture the job ID from sbatch output (typically "Submitted batch job <id>")
        lines.append(
            f'{job_var}=$(sbatch {dep_opt}<< "EOF" | awk \'{{print $4}}\')'
        )
        lines.append("#!/usr/bin/env bash")
        lines.append("set -euo pipefail")

        # If there are no steps, create a noop placeholder step
    ...
    lines.append('echo "All SLURM jobs submitted."')
    return "\n".join(lines).rstrip() + "\n"
